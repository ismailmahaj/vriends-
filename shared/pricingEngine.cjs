/**
 * Moteur de prix Vriends POS — source unique frontend + backend.
...
function formatCents(cents, locale = 'fr-BE') {
...
function resolveCustomerDiscountPercent(customerType, settings) {
...
/**
 * @param {object} input
 * @param {Array<{ unitPriceCents: number, quantity: number }>} input.items
 * @param {string} input.customerType
 * @param {Date|string|number} [input.now]
 * @param {object} [input.settings]
 */
function calculateOrderPricing({ items = [], customerType = CUSTOMER_TYPES.STANDARD, now = new Date(), settings = {} } = {}) {
  const s = { ...DEFAULT_POS_SETTINGS, ...settings };
  const appliedRules = [];
 ...
 */
function roundCashUpToTenCents(cents) {
...
function getPayableTotalCents(totalCents, paymentMethod) {
