/**
 * Wrapper ESM du moteur d'options (même logique que backend/lib/optionsEngine.cjs).
 * Dupliqué volontairement pour éviter les soucis CJS/ESM Vite.
 */
...
function normalizeGroup(raw, index = 0) {
  ...
  const choices = choicesRaw.map((c, i) => normalizeChoice(c, i)).filter(Boolean);
  const seen = new Set();
  const uniqueChoices = [];
  ...
  const selection =
    raw.selection === 'multiple' || raw.type === 'multiple' || raw.multi ? 'multiple' : 'single';
...
export function normalizeOptionsSchema(raw) {
  ...
  const groups = value.map((g, i) => normalizeGroup(g, i)).filter(Boolean);
  const seen = new Set();
  const unique = [];
...
export function computeUnitPriceEuros(basePrice, schema, selection) {
  return Math.round((asNumber(basePrice, 0) + computeOptionsExtraEuros(schema, selection)) * 100) / 100;
}

export function buildOptionsSnapshot(schema, selection) {
  const groups = normalizeOptionsSchema(schema) || [];
  const snapshot = [];
  for (const group of groups) {
    const ids = getSelectedIds(selection, group.id);
...
export function formatOptionsLabel(snapshotOrSelection, schema) {
  ...
  for (const group of groups) {
    ...
    const labels = ids.map((id) => group.choices.find((c) => c.id === id)?.label).filter(Boolean);
    if (labels.length) parts.push(`${group.name}: ${labels.join(', ')}`);
  }
  return parts.join(' · ');
}

export function schemaToEditorRows(schema) {
...
export function parseChoicesText(text) {
  return String(text || '')
    .split(',')
    .map((part) => part.trim())
    .filter(Boolean)
    .map((part, i) => {
      ...
      if (eq > 0) {
        return normalizeChoice({ label: part.slice(0, eq).trim(), priceDelta: asNumber(part.slice(eq + 1), 0) }, i);
      }
...
export function editorRowsToSchema(rows) {
